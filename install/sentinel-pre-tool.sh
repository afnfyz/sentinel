...
curl -s "http://localhost:49152/wait-approval?id=claude" > /tmp/sentinel-approval-claude &
CURL_PID=$!
...
if [ -n "$TERMINAL_INPUT" ]; then
  ...
  wait $CURL_PID
  MASCOT_ACTION=$(cat /tmp/sentinel-approval-claude | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('action','timeout'))" 2>/dev/null || echo "timeout")
